...
// --- Fonctions auxiliaires (placées dans le même scope pour être accessibles) ---
 ...
 */
 function creerPersonnages(keywordsList, univers, randomSeed) {
    ...
    const personnages = [];
    const archetypesPossibles = [...archetypes];

     // Assurer un héros et un antagoniste
    ...
    for (let i = 2; i < nombrePersonnages; i++) {
        if (archetypesPossibles.length === 0) break; // Sécurité si on manque d'archétypes
        const archetypeIndex = Math.floor(pseudoRandom(randomSeed + 6 + i) * archetypesPossibles.length);
        const selectedArchetype = archetypesPossibles.splice(archetypeIndex, 1)[0];
...
function creerPersonnageDetaille(archetype, univers, keywordsList, randomSeed, nomsParUnivers, allTraits, allMotivations) {
    ...
    // Assurer que nomsUnivers est un tableau avant d'accéder à .length
    const nom = Array.isArray(nomsUnivers) && nomsUnivers.length > 0
        ? nomsUnivers[Math.floor(pseudoRandom(randomSeed) * nomsUnivers.length)]
        : `Perso_${archetype}`; // Nom par défaut si problème
...
function creerStructureNarrative(keywordsList, univers, personnages, randomSeed) {
    const structures = [
     ...
     if (detectedStructure === "quête initiatique" && pseudoRandom(randomSeed + 9) > 0.3) { // 70% chance de choisir aléatoirement si pas détecté
    ...
    const pointsCleParStructure = {
     ...
     const etapes = pointsCleParStructure[detectedStructure] || pointsCleParStructure["quête initiatique"];
     ...
     for(let i=0; i<nombreChapitres; i++){
         ...
         const fin = Math.min(debut + etapesParChapitre, etapes.length);
          // Gérer le cas où fin <= debut (ne devrait pas arriver avec Math.ceil, mais par sécurité)
          if (fin > debut) {
            etapesDistribuees.push(etapes.slice(debut, fin).join(' / '));
          } else if (i === nombreChapitres -1 && etapes.length > 0) {
            // Si c'est le dernier chapitre et qu'il reste des étapes non assignées (peu probable)
            etapesDistribuees.push(etapes.slice(debut).join(' / '));
          } else {
             etapesDistribuees.push("Transition"); // Chapitre de transition si vide
          }
     }
    ...
    return {
        ...
        etapesParChapitre: etapesDistribuees,
...
function genererChapitresDetailles(structureNarrative, univers, personnages, randomSeed) {
    ...
    const etapesChapitres = structureNarrative.etapesParChapitre;
    const pagesParChapitre = 12;
    let numeroPageGlobal = 1;

    for (let i = 0; i < nombreChapitres; i++) {
        const etapeCourante = etapesChapitres[i] || `Chapitre ${i+1}`; // Fallback si étape vide
        let titreChapitre = etapeCourante.length > 40 ? `Chapitre ${i+1}` : etapeCourante; // Titre plus court si étape longue
        ...
        chapitres.push({
            numero: i + 1,
            titre: `Chapitre ${i+1}: ${titreChapitre}`,
            etape: etapeCourante,
            pages: pages,
            resume: `Ce chapitre se concentre sur "${etapeCourante}", faisant progresser l'intrigue.`
        });
    }
    return chapitres;
}
...
function genererPage(numeroGlobal, numeroChapitre, numeroLocal, univers, personnages, etapeChapitre, randomSeed) {
    // Nombre de cases plus variable
     const baseCases = 4;
     const variation = Math.floor(pseudoRandom(randomSeed) * 5) - 2; // -2 à +2 cases
     const nombreCases = Math.max(3, Math.min(7, baseCases + variation)); // Entre 3 et 7 cases
    ...
    for (let k = 0; k < nombreCases; k++) {
         ...
         // Logique de sélection améliorée :倾向于 inclure le héros, parfois l'antagoniste, parfois juste des alliés
         const probaHero = 0.6;
         const probaAntago = 0.3;
         const maxPersoCase = 3; // Max 3 persos par case pour clarté
         const nbPersoCase = Math.floor(pseudoRandom(randomSeed + k + 1) * maxPersoCase) + 1; // 1 à 3 persos

          // Essayer d'ajouter le héros
          const hero = personnages.find(p => p.archetype === 'héros');
          if (hero && pseudoRandom(randomSeed + k + 2) < probaHero) {
               personnagesPresents.push(hero);
          }
           // Essayer d'ajouter l'antagoniste (moins probable, sauf si étape clé)
           const antago = personnages.find(p => p.archetype === 'antagoniste');
           if (antago && !personnagesPresents.includes(antago) && personnagesPresents.length < nbPersoCase &&
               pseudoRandom(randomSeed + k + 3) < probaAntago) {
                personnagesPresents.push(antago);
           }
            // Compléter avec d'autres personnages
            const persosRestants = personnages.filter(p => !personnagesPresents.includes(p));
            while(personnagesPresents.length < nbPersoCase && persosRestants.length > 0) {
                 const index = Math.floor(pseudoRandom(randomSeed + k + 4 + personnagesPresents.length) * persosRestants.length);
                 personnagesPresents.push(persosRestants.splice(index, 1)[0]);
            }
            // Si toujours vide (impossible normalement avec le fallback ci-dessous)
             if (personnagesPresents.length === 0 && personnages.length > 0) {
                  personnagesPresents.push(personnages[0]); // Fallback absolu
             }
    ...
    return {
        ...
        cases: cases,
        description: `Page ${numeroGlobal} (Chap. ${numeroChapitre}, Pg. ${numeroLocal}). Focus: "${etapeChapitre}".`
    };
}
 ...
 */
 function genererCase(numeroPage, numeroCase, univers, personnagesPresents, etapeChapitre, randomSeed) {
    ...
    const lieu = univers.lieux && univers.lieux.length > 0 ? univers.lieux[Math.floor(pseudoRandom(randomSeed + 1) * univers.lieux.length)] : "Lieu Indéfini";
     ...
     let dialogue = null;
     if (personnagesPresents.length > 0 && pseudoRandom(randomSeed + 3) > 0.45) { // 55% de chance
     ...
     const narration = pseudoRandom(randomSeed + 6) > 0.7 ? genererNarration(univers, lieu, personnagesPresents, etapeChapitre, dialogue, randomSeed + 6) : null; // Narration moins fréquente
    ...
    return {
        ...
        typePlan: typePlan,
        lieu: lieu,
        personnagesPresents: personnagesPresents.map(p => p.nom),
        dialogue: dialogue,
        narration: narration,
 ...
 function genererLigneDialogue(personnage, etape, seed) {
     const baseDialogues = [
         ...
         "Plus vite !", "Où sommes-nous ?", "Je crois que j'ai compris.", "Impossible...", "Suivez-moi !", "Silence !"
     ];
     const dialoguesParArchetype = {
        "héros": ["Je ne laisserai personne derrière !", "Pour la justice !", "Je peux le faire.", "Quel est le plan ?", "Nous devons essayer !"],
        "mentor": ["La patience est une vertu.", "Souviens-toi de ton entraînement.", "Le destin t'attend.", "Observe et apprends.", "Ne sous-estime jamais ton adversaire."],
        "allié": ["Compte sur moi !", "J'ai un mauvais pressentiment...", "On se couvre, ok ?", "C'était moins une !", "Je m'en occupe."],
        "antagoniste": ["Pathétique.", "Vous ne pouvez pas m'arrêter !", "Le pouvoir sera mien !", "Comme c'est prévisible.", "Agenouillez-vous !"],
        "ombre": ["Tout a un prix...", "Peut-être... peut-être pas.", "Laissez-moi tranquille.", "Ça ne vous regarde pas.", "Les choses ne sont pas si simples."],
        "gardien": ["Halte ! Qui va là ?", "Vous ne passerez pas !", "Seuls les dignes...", "Respectez ce lieu.", "Le savoir a un prix."],
        "messager": ["J'ai un message pour vous.", "Écoutez attentivement.", "Le temps presse.", "Les choses ont changé.", "On m'envoie vous prévenir."],
        "caméléon": ["Intéressant...", "Comment puis-je vous aider... pour l'instant ?", "Cela pourrait être profitable.", "Je connais quelqu'un...", "Adaptons-nous."]
     };
      // Dialogues spécifiques à l'étape (exemples)
      const dialoguesParEtape = {
          "Monde Ordinaire": ["Encore une journée ordinaire...", "J'aimerais qu'il se passe quelque chose."],
          "Appel & Refus": ["Moi ? Pourquoi moi ?", "C'est impossible, je ne peux pas."],
          "Mentor & Seuil": ["Vous devez accepter votre destin.", "Le premier pas est le plus difficile."],
          "Épreuves & Alliés": ["Nous devons travailler ensemble !", "Fais attention, c'est un piège !"],
          "Ordalie": ["C'est maintenant ou jamais !", "Je ne peux pas perdre..."],
          "Retour & Résurrection": ["Je suis de retour.", "Les choses ont changé ici."],
           "Le Crime/Mystère": ["Qui aurait pu faire ça ?", "Il y a quelque chose qui cloche."],
           "Fausses Pistes": ["Je croyais qu'on le tenait...", "Retour à la case départ."],
            "Révélation Choc": ["Alors c'était vous depuis le début !", "Je n'arrive pas à y croire..."],
             "Vie sous l'Oppression": ["Baissez la tête et ne faites pas de vagues.", "Un jour, ça changera."],
              "Rejoindre la Résistance": ["Je veux me battre.", "Que puis-je faire ?"],
               "Bataille Décisive": ["Pour la liberté !", "C'est notre dernière chance !"]
           // ... ajouter pour d'autres étapes clés ...
      };
       // Sélectionner une source de dialogue
       let sourceDialogues = baseDialogues;
       if (pseudoRandom(seed) < 0.6) { // 60% chance d'utiliser un dialogue d'archétype
           sourceDialogues = [...baseDialogues, ...(dialoguesParArchetype[personnage.archetype] || [])];
       }
       // Ajouter éventuellement un dialogue basé sur l'étape actuelle
        const etapeSimple = etape.split(' / ')[0]; // Prendre la première étape si groupées
        if (dialoguesParEtape[etapeSimple] && pseudoRandom(seed + 0.5) < 0.4) { // 40% chance d'ajouter dialogue d'étape
             sourceDialogues = [...sourceDialogues, ...dialoguesParEtape[etapeSimple]];
        }
  ...
  function genererNarration(univers, lieu, personnages, etape, dialogue, seed) {
     const baseNarrations = [
         "Le silence était lourd.", "Une tension palpable.", "Le temps suspendu.",
         `L'atmosphère de ${lieu.toLowerCase()}.`, "Soudain...", "Pendant ce temps...",
         "Au loin...", "Les ombres dansaient.", "Urgence.", "Non-dits.", "L'air vibrait."
     ];
      // Ajouter des narrations spécifiques
      let possibles = [...baseNarrations];
      if(personnages.length === 1) possibles.push(`${personnages[0].nom} était seul(e) avec ses pensées.`);
      if(etape.includes("Obstacle") || etape.includes("Confrontation")) possibles.push("Le danger rôdait.");
  ...
  function genererDescriptionVisuelle(typePlan, lieu, personnages, etape, dialogue, narration, seed) {
     let desc = `${typePlan.charAt(0).toUpperCase() + typePlan.slice(1)} dans ${lieu}. `; // Capitaliser le type de plan

     if (personnages.length > 0) {
         const noms = personnages.map(p => p.nom).join(', ');
         desc += `${noms}. `;
         // Décrire action/expression du premier perso
          const actions = [
              `regarde ${pseudoRandom(seed) > 0.5 ? 'intensément' : 'vaguement'} ${pseudoRandom(seed+1) > 0.5 ? 'devant lui/elle' : 'vers quelque chose hors champ'}`,
              `a une expression ${['inquiète', 'déterminée', 'surprise', 'fatiguée', 'méfiante'][Math.floor(pseudoRandom(seed+2)*5)]}`,
              `examine ${['un objet au sol', 'une carte holographique', 'ses mains tremblantes', 'le mécanisme complexe'][Math.floor(pseudoRandom(seed+3)*4)]}`,
              `se tient ${['prêt(e) à bondir', 'immobile, à l'écoute', 'légèrement en retrait', 'au centre de l'attention'][Math.floor(pseudoRandom(seed+4)*4)]}`,
              `semble ${['parler animement (si dialogue)', 'murmurer pour lui/elle-même', 'réfléchir profondément', 'ignorer les autres'][Math.floor(pseudoRandom(seed+5)*4)]}${dialogue ? '' : ' (sans dialogue)'}.`
          ];
         ...
         desc += `Le lieu est vide, soulignant ${['sa désolation', 'son immensité', 'un détail important', 'le calme avant la tempête'][Math.floor(pseudoRandom(seed+8)*4)]}. `;
     }

     const ambiances = ["Lumière crépusculaire filtrant par une fenêtre.", "Ombres portées créant une atmosphère oppressante.", "Brume épaisse réduisant la visibilité.", "Éclairage néon vibrant et froid.", "Poussière flottant dans un rayon de soleil."];
