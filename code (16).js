...
window.generateScenarioDetaille = async function(keywords) {
    try {
        ...
        let title = genererTitreCreatif(keywordsList, randomSeed);
         // console.log("generateScenarioDetaille: Titre généré: ", title); // Optionnel

        // Création d'un univers cohérent basé sur les mots-clés
        const univers = creerUnivers(keywordsList, randomSeed);
         // console.log("generateScenarioDetaille: Univers généré: ", univers); // Optionnel

        // Création des personnages principaux
        const personnages = creerPersonnages(keywordsList, univers, randomSeed);
         // console.log("generateScenarioDetaille: Personnages générés: ", personnages); // Optionnel
        ...
        const structureNarrative = creerStructureNarrative(keywordsList, univers, personnages, randomSeed);
         // console.log("generateScenarioDetaille: Structure narrative générée: ", structureNarrative); // Optionnel
        ...
        const chapitres = genererChapitresDetailles(structureNarrative, univers, personnages, randomSeed);
         // console.log("generateScenarioDetaille: Chapitres générés: ", chapitres); // Optionnel
        ...
        // *** POINT DE CONTRÔLE EN CAS D'ERREUR ***
        ...
        // alert("Une erreur interne est survenue lors de la génération du scénario. Vérifiez la console."); // Redondant avec l'alerte ci-dessus
        return null; // Renvoyer null en cas d'erreur
    }
}

// --- Fonctions auxiliaires (inchangées, juste incluses pour que le fichier soit complet) ---
...
function creerPersonnageDetaille(archetype, univers, keywordsList, randomSeed, nomsParUnivers, allTraits, allMotivations) {
    ...
    const nombreTraits = Math.floor(pseudoRandom(randomSeed + 1) * 2) + 2;
...
function creerStructureNarrative(keywordsList, univers, personnages, randomSeed) {
    ...
    let detectedStructure = "quête initiatique";
    ...
    if (detectedStructure === "quête initiatique" && pseudoRandom(randomSeed + 9) > 0.3) { detectedStructure = structures[Math.floor(pseudoRandom(randomSeed + 10) * structures.length)]; }
...
function genererChapitresDetailles(structureNarrative, univers, personnages, randomSeed) {
    ...
    for (let i = 0; i < nombreChapitres; i++) {
        const etapeCourante = etapesChapitres[i] || `Chapitre ${i+1}`;
        let titreChapitre = etapeCourante.length > 40 ? `Chapitre ${i+1}` : etapeCourante;
        const pages = [];
        for (let j = 0; j < pagesParChapitre; j++) { pages.push(genererPage(numeroPageGlobal, i + 1, j + 1, univers, personnages, etapeCourante, randomSeed + numeroPageGlobal)); numeroPageGlobal++; }
...
function genererCase(numeroPage, numeroCase, univers, personnagesPresents, etapeChapitre, randomSeed) {
    ...
    let dialogue = null; if (personnagesPresents.length > 0 && pseudoRandom(randomSeed + 3) > 0.45) { const persoQuiParle = personnagesPresents[Math.floor(pseudoRandom(randomSeed + 4) * personnagesPresents.length)]; dialogue = { personnage: persoQuiParle.nom, texte: genererLigneDialogue(persoQuiParle, etapeChapitre, randomSeed + 5) }; }
