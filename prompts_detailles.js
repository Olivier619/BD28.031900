...
window.generatePromptsDetailles = async function(storyboard) {
    ...
    try {
        ...
        // Styles et éléments visuels (condensé, assurez-vous d'avoir les vôtres)
        const artStylesParUnivers = { "contemporain": ["modern illustration style", "realistic contemporary art"], /* ... autres ... */ };
        const elementsVisuelsDeBase = ["detailed", "high quality", "cinematic lighting", "dynamic composition"];
        const elementsVisuelsSpecifiques = { "contemporain": ["urban environment", "modern architecture"], /* ... autres ... */ };
        ...
        for (const page of storyboard.pages) {
            ...
            for (const [index, caseItem] of page.cases.entries()) {
                ...
                let artStyles = artStylesParUnivers[univers.type] || artStylesParUnivers["contemporain"] || ["illustration style"]; // Fallback style
                const style = artStyles[Math.floor(pseudoRandom(Date.now() + index * 10) * artStyles.length)];
                const elementsDeBase = selectionnerElementsAleatoires(elementsVisuelsDeBase, 2, 3, Date.now() + index * 11); // Moins d'éléments
                const elementsSpecifiques = selectionnerElementsAleatoires(elementsVisuelsSpecifiques[univers.type] || elementsVisuelsSpecifiques["contemporain"] || [], 1, 2, Date.now() + index * 12); // Moins d'éléments
                ...
                const promptComplet = `${style}, ${elementsDeBase.join(", ")}, ${elementsSpecifiques.join(", ")}, ${promptDescription} --ar 16:9 --style raw`.replace(/, ,/g, ',').replace(/, $/,''); // Nettoyage final
                // console.log(`Prompt P${page.pageNumber}-C${index+1}: ${promptComplet.substring(0,100)}...`); // Optionnel
...
// --- Fonctions auxiliaires (condensées, ASSUREZ-VOUS D'AVOIR VOS VERSIONS COMPLÈTES) ---
