 ...
 * VERSION CORRIGÉE pour gérer les types d'univers non définis dans caracteristiques
 */
function creerUnivers(keywordsList, randomSeed) {
    ...
    for (const type in typeKeywords) {
        if (keywordsList.some(kw => typeKeywords[type].includes(kw))) {
            detectedType = type;
            console.log("creerUnivers: Type détecté par mot-clé:", detectedType);
            break;
        }
    }
    // Choix aléatoire si non détecté et assez de mots-clés
    if (detectedType === "contemporain" && keywordsList.length > 2) {
        const randomIndex = Math.floor(pseudoRandom(randomSeed + 2) * typesUnivers.length);
        detectedType = typesUnivers[randomIndex];
        console.log("creerUnivers: Type choisi aléatoirement:", detectedType);
    }

    // !! ASSUREZ-VOUS QUE CET OBJET CONTIENT TOUTES LES ENTRÉES POUR LES TYPES CI-DESSUS !!
    ...
    if (!caracteristiques[detectedType]) {
        console.warn(`creerUnivers: Type d'univers "${detectedType}" non trouvé dans les caractéristiques. Utilisation de "contemporain" par défaut.`);
        finalType = "contemporain"; // Utiliser le type par défaut
    }
    // ****************************

    const universData = caracteristiques[finalType]; // Utiliser finalType ici

    if (!universData) {
        console.error("creerUnivers: ERREUR CRITIQUE - Impossible de trouver les données pour le type d'univers final:", finalType);
        return { type: finalType, description: "Erreur lors de la génération des détails de l'univers.", lieux: ["Lieu par Défaut"] };
    }

    return {
        type: finalType,
        ...universData,
        description: `Un univers ${finalType} ${universData.epoque ? `se déroulant vers ${universData.epoque}` : ''}. Les particularités incluent : ${universData.particularites}. Technologie ${universData.technologie}.`
    };
}
