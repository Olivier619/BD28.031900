...
function genererChapitresDetailles(structureNarrative, univers, personnages, randomSeed) {
    ...
    for (let i = 0; i < nombreChapitres; i++) {
        ...
        const modeleIndex = Math.floor((randomSeed % (1100 + i * 100)) / (1100 + i * 100) * modelesTitres.length);
        let titre = modelesTitres[modeleIndex];
        
        // Remplacer les placeholders
        titre = titre.replace("[N]", (i + 1).toString());
        
        // Sélectionner une étape de la structure narrative
        const etapeIndex = Math.floor((i / nombreChapitres) * etapes.length);
        const etape = etapes[etapeIndex];
        titre = titre.replace("[Étape]", etape.toLowerCase());
        
        // Générer les pages du chapitre
        const pages = [];
        
        for (let j = 0; j < pagesParChapitre; j++) {
            // Numéro de page global (1-48)
            const numeroPage = i * pagesParChapitre + j + 1;
            
            // Générer le contenu de la page
            const page = genererPage(numeroPage, i + 1, j + 1, univers, personnages, etape, randomSeed + numeroPage);
            pages.push(page);
        }
        
        // Créer l'objet chapitre
        const chapitre = {
            numero: i + 1,
            titre: titre,
            etape: etape,
            pages: pages,
            resume: `Dans ce chapitre, nous suivons les personnages à travers l'étape "${etape}" de leur parcours.`
        };
 ...
 */
function genererPage(numeroGlobal, numeroChapitre, numeroLocal, univers, personnages, etape, randomSeed) {
    // Nombre de cases par page (3-6)
    const nombreCases = Math.floor((randomSeed % 1200) / 1200 * 4) + 3;
    
    // Générer les cases de la page
    const cases = [];
    
    for (let i = 0; i < nombreCases; i++) {
        // Générer le contenu de la case
        const caseDetail = genererCase(numeroGlobal, numeroChapitre, numeroLocal, i + 1, univers, personnages, etape, randomSeed + i);
    ...
    const page = {
        ...
        cases: cases,
        description: `Page ${numeroGlobal} (Chapitre ${numeroChapitre}, Page ${numeroLocal}): Contient ${nombreCases} cases illustrant une partie de l'étape "${etape}".`
    };
 ...
 */
function genererCase(numeroPage, numeroChapitre, numeroLocalPage, numeroCase, univers, personnages, etape, randomSeed) {
    // Types de plans possibles
    const typesPlans = [
        "plan large", "plan moyen", "gros plan", "très gros plan", 
        "plan d'ensemble", "plongée", "contre-plongée", "plan américain"
    ];
    
    // Sélectionner un type de plan
    const planIndex = Math.floor((randomSeed % 1300) / 1300 * typesPlans.length);
    const typePlan = typesPlans[planIndex];
    
    // Sélectionner un lieu de l'univers
    const lieuIndex = Math.floor((randomSeed % 1400) / 1400 * univers.lieux.length);
    const lieu = univers.lieux[lieuIndex];
    
    // Sélectionner des personnages présents dans la case
    const nombrePersonnagesPresents = Math.floor((randomSeed % 1500) / 1500 * 3) + 1; // 1-3 personnages
    const personnagesPresents = [];
    
    for (let i = 0; i < Math.min(nombrePersonnagesPresents, personnages.length); i++) {
        const personnageIndex = Math.floor((randomSeed % (1600 + i * 100)) / (1600 + i * 100) * personnages.length);
        personnagesPresents.push(personnages[personnageIndex].nom);
    }
    
    // Générer un dialogue (50% de chances d'avoir un dialogue)
    let dialogue = null;
    if ((randomSeed % 1700) / 1700 < 0.5 && personnagesPresents.length > 0) {
        const personnageQuiParle = personnagesPresents[Math.floor((randomSeed % 1800) / 1800 * personnagesPresents.length)];
        
        // Dialogues possibles selon l'étape narrative
        const dialoguesPossibles = [
            `"Je ne comprends pas ce qui se passe ici."`,
            `"Nous devons continuer, quoi qu'il arrive."`,
            `"Je n'aurais jamais cru voir ça un jour."`,
            `"Est-ce que quelqu'un a un plan ?"`,
            `"Cet endroit me donne la chair de poule."`,
            `"Nous n'avons pas le choix, il faut avancer."`,
            `"Je sens que nous sommes observés."`,
            `"Qu'est-ce que c'était que ce bruit ?"`,
            `"Regardez ce que j'ai trouvé !"`,
            `"Je crois que nous ne sommes pas seuls."`
        ];
        
        const dialogueIndex = Math.floor((randomSeed % 1900) / 1900 * dialoguesPossibles.length);
        dialogue = {
            personnage: personnageQuiParle,
            texte: dialoguesPossibles[dialogueIndex]
        };
    }
    
    // Générer une narration
    const narrationsPossibles = [
        "Le silence règne, lourd et oppressant.",
        "Une atmosphère de tension est palpable.",
        "L'obscurité semble vivante, mouvante.",
        "La lumière révèle des détails inquiétants.",
        "Un sentiment d'urgence envahit les personnages.",
        "Le temps semble suspendu dans cet instant crucial.",
        "Les regards se croisent, chargés de non-dits.",
        "L'environnement hostile accentue le danger.",
        "Un indice crucial apparaît, presque imperceptible.",
        "La révélation change tout ce qu'ils croyaient savoir."
    ];
    
    const narrationIndex = Math.floor((randomSeed % 2000) / 2000 * narrationsPossibles.length);
    const narration = narrationsPossibles[narrationIndex];
    
    // Créer la description visuelle de la case
    const descriptions = [
        `${typePlan} sur ${lieu}, où ${personnagesPresents.join(", ")} font face à un nouveau défi.`,
        `${typePlan} montrant ${personnagesPresents.join(", ")} dans ${lieu}, avec une ambiance tendue.`,
        `${typePlan} de ${lieu}, avec ${personnagesPresents.join(", ")} en pleine action.`,
        `${typePlan} révélant ${personnagesPresents.join(", ")} découvrant un secret dans ${lieu}.`,
        `${typePlan} capturant l'expression de ${personnagesPresents.join(", ")} alors qu'ils explorent ${lieu}.`
    ];
    
    const descriptionIndex = Math.floor((randomSeed % 2100) / 2100 * descriptions.length);
    const descriptionVisuelle = descriptions[descriptionIndex];
    ...
    const caseDetail = {
        ...
        typePlan: typePlan,
        lieu: lieu,
        personnagesPresents: personnagesPresents,
        dialogue: dialogue,
        narration: narration,
