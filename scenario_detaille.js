...
window.generateScenarioDetaille = async function(keywords) {
    try {
        ...
        console.log("generateScenarioDetaille: Scénario final généré.");
        // console.log('>>> scenario_detaille.js: Prêt à retourner le scénario :', scenario); // Optionnel
        await new Promise(resolve => setTimeout(resolve, 50));
        return scenario;

    } catch (error) {
        console.error("generateScenarioDetaille: ERREUR lors de la génération:", error);
        // L'alerte est maintenant gérée dans le .catch de main.js
...
// --- Fonctions auxiliaires (DOIVENT TOUTES ÊTRE PRÉSENTES) ---
...
function creerUnivers(keywordsList, randomSeed) {
    ...
    const caracteristiques = { /* ... (VOTRE OBJET CARACTERISTIQUES COMPLET) ... */
    ...
    if (!caracteristiques[detectedType]) { console.warn(`creerUnivers: Type "${detectedType}" inconnu. Fallback.`); finalType = "contemporain"; }
    const universData = caracteristiques[finalType];
    if (!universData) { console.error("creerUnivers: ERREUR données introuvables pour:", finalType); return { type: finalType, description: "Erreur.", lieux: ["?"] }; }
...
function genererChapitresDetailles(structureNarrative, univers, personnages, randomSeed) {
    ...
    // console.log(`>>> genererChapitresDetailles: Démarrage pour ${nombreChapitres} chapitres`); // Log ajouté
    for (let i = 0; i < nombreChapitres; i++) {
        ...
        // console.log(`>>> genererChapitresDetailles: Génération Chapitre ${i + 1} (${titreChapitre})`); // Log ajouté
        const pages = [];
        ...
        chapitres.push({ numero: i + 1, titre: `Chapitre ${i+1}: ${titreChapitre}`, etape: etapeCourante, pages: pages, resume: `Ce chapitre se concentre sur "${etapeCourante}".` });
    }
    // console.log(">>> genererChapitresDetailles: Tableau chapitres final:", chapitres); // Log ajouté
    return chapitres;
 }

function genererPage(numeroGlobal, numeroChapitre, numeroLocal, univers, personnages, etapeChapitre, randomSeed) {
    // console.log(`>>> genererPage: Génération Page Glob ${numeroGlobal} (Chap ${numeroChapitre}-${numeroLocal})`); // Log ajouté
    const baseCases = 4; const variation = Math.floor(pseudoRandom(randomSeed) * 5) - 2; const nombreCases = Math.max(3, Math.min(7, baseCases + variation));
    const cases = [];
    ...
    const page = { numeroGlobal: numeroGlobal, numeroChapitre: numeroChapitre, numeroLocal: numeroLocal, cases: cases, description: `Page ${numeroGlobal} (Chap. ${numeroChapitre}, Pg. ${numeroLocal}). Focus: "${etapeChapitre}".` };
    // console.log(`>>> genererPage: Page ${numeroGlobal} générée:`, page); // Log ajouté
    return page;
}

function genererCase(numeroPage, numeroCase, univers, personnagesPresents, etapeChapitre, randomSeed) {
    // console.log(`>>> genererCase: Appel pour Page ${numeroPage}, Case ${numeroCase}`, { numeroCaseType: typeof numeroCase, personnagesPresents }); // Log ajouté
    if (typeof numeroCase !== 'number' || isNaN(numeroCase)) { console.error(`>>> genererCase: ERREUR - numeroCase invalide reçu ! Reçu:`, numeroCase); return { numeroPage, numeroCase: 'ERREUR_NUM', typePlan: 'erreur', lieu: 'erreur', personnagesPresents: [], dialogue: null, narration: null, descriptionVisuelle: 'Erreur interne - numeroCase invalide' }; }
    ...
    if (Array.isArray(personnagesPresents) && personnagesPresents.length > 0 && pseudoRandom(randomSeed + 3) > 0.45) {
        ...
        if (persoQuiParle) {
           dialogue = { personnage: persoQuiParle.nom || "?", texte: genererLigneDialogue(persoQuiParle, etapeChapitre, randomSeed + 5) };
        } else { console.warn(`>>> genererCase: Impossible sélectionner persoQuiParle (P${numeroPage}-C${numeroCase})`); }
    }
    ...
    return { numeroPage: numeroPage, numeroCase: numeroCase, typePlan: typePlan, lieu: lieu, personnagesPresents: personnageNoms, dialogue: dialogue, narration: narration, descriptionVisuelle: descriptionVisuelle };
 }
...
function genererLigneDialogue(personnage, etape, seed) {
    ...
    return sourceDialogues.length > 0 ? sourceDialogues[Math.floor(pseudoRandom(seed + 1) * sourceDialogues.length)] : "..."; // Fallback
}
...
function genererDescriptionVisuelle(typePlan, lieu, personnages, etape, dialogue, narration, seed) {
    ...
    const ambiances = ["Lumière crépusculaire.", "Ombres portées.", "Brume épaisse.", "Éclairage néon.", "Poussière en suspension."]; // Simplifié
